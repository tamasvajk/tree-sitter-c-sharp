...
// https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/language-specification/expressions#1142-operator-precedence-and-associativity
const PREC = {
  PRIMARY: 17,
  UNARY: 16,
  RANGE: 15,
  ...
  ADDITIVE: 12,
  SHIFT: 11,
  RELATIONAL_AND_TYPE_TESTING: 10,
  EQUALITY: 9,
  BITWISE_AND: 8,
  BITWISE_XOR: 7,
  BITWISE_OR: 6,
  CONDITIONAL_AND: 5,
  CONDITIONAL_OR: 4,
  NULL_COALESCING_AND_THROW: 3,
  CONDITIONAL: 2,
  ASSIGNMENT_AND_LAMBDA_DECLARATION: 1
};
...
module.exports = grammar({
  ...
  rules: {
    ...
    extern_alias_directive: $ => seq('extern', 'alias', $._identifier_token, ';'),
    ...
    global_attribute_list: $ => seq(
      '[',
      alias($.global_attribute_target_specifier, $.attribute_target_specifier),
      commaSep1($.attribute),
      optional(','),
      ']'
    ),

    global_attribute_target_specifier: $ => seq(
    ...
    // This can represent both nullable value and reference types.
    ...
    function_pointer_parameter_list: $ => seq(
      ...
      alias($._type, $.function_pointer_parameter),
      '>'
    ),

    function_pointer_calling_convention: $ => choice(
      'managed', // function_pointer_unmanaged_calling_convention_list? is present in Roslyn.
      seq(
        'unmanaged',
    ...
    function_pointer_unmanaged_calling_convention: $ => $._identifier_token,
    ...
    // `prec.dynamic` added to resolve ambiguity between `(a)(b)`, which is a cast `(a)b` and not an invocation `a(b)`
    cast_expression: $ => prec(PREC.UNARY, prec.dynamic(1, seq(
      '(',
      field('type', $._type),
      ')',
      field('value', $._expression)
    ...
    invocation_expression: $ => prec(PREC.PRIMARY, seq(
    ...
    size_of_expression: $ => prec(PREC.PRIMARY, seq(
      'sizeof',
      '(',
      $._type,
      ')'
    ...
    throw_expression: $ => prec.right(PREC.NULL_COALESCING_AND_THROW, seq('throw', $._expression)),

    tuple_expression: $ => prec(PREC.PRIMARY, seq(
      '(',
      $.argument,
      repeat1(seq(
        ',',
    ...
    type_of_expression: $ => prec(PREC.PRIMARY, seq('typeof', '(', $._type, ')')),

    with_expression: $ => prec.left(PREC.SWITCH_AND_WITH,
      seq($._expression, 'with', $.initializer_expression)),
    ...
    binary_expression: $ => choice(
      ...[
        ['&&', PREC.CONDITIONAL_AND], // logical_and_expression
        ['||', PREC.CONDITIONAL_OR], // logical_or_expression
        ['>>', PREC.SHIFT], // right_shift_expression
        ['<<', PREC.SHIFT], // left_shift_expression
        ['&', PREC.BITWISE_AND],  // bitwise_and_expression
        ['^', PREC.BITWISE_XOR], // exclusive_or_expression
        ['|', PREC.BITWISE_OR], // bitwise_or_expression
        ['+', PREC.ADDITIVE], // add_expression
        ['-', PREC.ADDITIVE], // subtract_expression
        ['*', PREC.MULTIPLICATIVE], // multiply_expression
        ['/', PREC.MULTIPLICATIVE], // divide_expression
        ['%', PREC.MULTIPLICATIVE], // modulo_expression
        ['<', PREC.RELATIONAL_AND_TYPE_TESTING], // less_than_expression
        ['<=', PREC.RELATIONAL_AND_TYPE_TESTING], // less_than_or_equal_expression
        ['==', PREC.EQUALITY], // equals_expression
        ['!=', PREC.EQUALITY], // not_equals_expression
        ['>=', PREC.RELATIONAL_AND_TYPE_TESTING], // greater_than_or_equal_expression
        ['>', PREC.RELATIONAL_AND_TYPE_TESTING] //  greater_than_expression
      ...
      prec.right(PREC.NULL_COALESCING_AND_THROW, seq(
        field('left', $._expression),
    ...
    as_expression: $ => prec.left(PREC.RELATIONAL_AND_TYPE_TESTING, seq(
      field('left', $._expression),
      field('operator', 'as'),
      field('right', $._type)
    )),

    is_expression: $ => prec.left(PREC.RELATIONAL_AND_TYPE_TESTING, seq(
      field('left', $._expression),
      field('operator', 'is'),
      field('right', $._type)
    )),
